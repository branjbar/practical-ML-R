...
```
### Loading data
Before loading data, make sure the working directory is pointing to where the *pml-training.csv* is located. It's important to translate the *NA* and *#DIV/0!* values to the *NA* value known by R. 
```{r echo=FALSE}
...
Here we split the *myData* into training and testing sets.
```{r}
...
We use a Decision Tree to find the informative featues.
```{r}
folds=2
repeats=2
...
Although, the decision tree is not very accurate, it helps us to find a smaller collection of very informative features. Thus, we redefine the *useful_features* as following.
``` {r}
...
```
Once more, we perform the KNN Imputation.
```{r}
